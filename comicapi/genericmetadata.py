...
class GenericMetadata:
    ...
    def assign_add_missing(self, cur: Any, new: Any) -> Any:
        """Add Missing - Any 'cur(rent)' values that are None or an empty str/list/set add 'new' non-None values"""
        if new is None:
            return cur
        if isinstance(cur, (list, set)) and len(cur) == 0:
            return new
        elif isinstance(cur, str) and len(cur) == 0:
            return new
        elif cur is None:
            return new
        else:
    ...
    def overlay(self, new_md: GenericMetadata, mode: OverlayMode = OverlayMode.overlay) -> None:
        ...
        self.tag_origin = assign(self.tag_origin, new_md.tag_origin)
